...
export default function UserUpdateContact(props: ComponentProps) {
  ...
  useEffect(() => {
    setUpdateAddress(user.userAddress ?? []);
    setUpdatePhone(user.userOtherPhone ?? []);
    setUpdateEmail(user.userOtherEmail ?? []);
  }, [user]);
      ...
      <div className="flex flex-col gap-3 p-2 mt-4">
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="flex gap-2 items-center">
            <button
              onClick={() => {
                getEmailValue(updateEmail);
                getAddressValue(updateAddress);
                getPhoneValue(updatePhone);
                submitChange(true);
              }}
